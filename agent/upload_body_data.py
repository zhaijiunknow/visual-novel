...
def get_existing_body_records(token):
    ...
    if result.get("code") == 0:
        for record in result.get("data", {}).get("items", []):
            fields = record.get("fields", {})
            if "角色-服装-动作" in fields:
                name = fields["角色-服装-动作"]["value"][0]["text"]
                existing.add(name)
...
def create_body_record(token, character_name, costume, action, role_record_id):
    ...
    fields = {
        ...
        "服装": costume,
        "动作": action,
        "角色-服装-动作": [{"text": f"{character_name}-{costume}-{action}", "type": "text"}],
        "角色名称": [{"text": character_name, "type": "text"}]
    }
    ...
    for filename in sorted(os.listdir(CHARACTERS_DIR)):
        if filename.startswith('character_') and filename.endswith('.tscn'):
            ...
            for anim in animations:
                ...
                # 检查是否已存在
                composite_name = f"{character_name}-{costume}-{action}"
                if composite_name in existing_records:
                    skipped_count += 1
                ...
                if result.get("code") == 0:
                    added_count += 1
                    existing_records.add(composite_name)
